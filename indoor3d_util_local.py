...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    for f in glob.glob(os.path.join(anno_path, '*.txt')):   # 找到所有匹配的路径
        cls = os.path.basename(f).split('_')[0]  # os.path.basename()返回文件名，最终返回分类名
    ...
    data_label[:, 0:3] -= xyz_min  # xyz normalization
...
def room2blocks(data, label, num_point, block_size=BLOCK_SIZE, stride=STRIDE,
    ...
    xbeg_list = []   # the start x coordinate of the block
    ybeg_list = []   # the start y coordinate of the block
    if not random_sample:
        num_block_x = int(np.ceil((limit[0] - block_size) / stride)) + 1  # np.ceil 向上取整
    ...
    for idx in range(len(xbeg_list)):
       ...
       cond = xcond & ycond
       if np.sum(cond) < min_points_block: # discard block if there are less than 100 pts.
           continue
       
       block_data = data[cond, :]    # block
       ...
       out_block_data = np.concatenate(block_data_list, 0)
       out_block_label = np.concatenate(block_label_list, 0)
            
    return out_block_data, out_block_label
...
def room2blocks_plus_normalized(data_label, num_point, block_size=BLOCK_SIZE, stride=STRIDE,
                                random_sample=False, sample_num=None, sample_aug=1):
    """ room2block, with input filename and RGB preprocessing.
        for each block centralize XYZ, add normalized XYZ as 678 channels
    """
    # filter_index = data_label[:, 2] > (-2.2)
    # data_label = data_label[filter_index, :]

    filter_mask = data_label[:, 2] > -2.2
    data_label = data_label[filter_mask, :]

    data = data_label[:, 0:4]
    label = data_label[:, -1].astype(np.uint8)
    min_xyz = np.min(data[:, 0:3], axis=0)  # shape=(3,)
    data[:, 0:3] -= min_xyz
    max_xyzi = np.max(data, axis=0)  # shape=(3,)
    # data[:, 0:3] += [x_range, y_range, z_range]
    # max_global_i = np.max(data[:, -1])
    # data[:, 3] /= max_global_i

    
    data_batch, label_batch = room2blocks(data, label, num_point, block_size, stride,
                                          random_sample, sample_num, sample_aug)
    # shape = (n, num_point, 4), (n, num_point)
    new_data_batch = np.zeros((data_batch.shape[0], num_point, 8))
    for b in range(data_batch.shape[0]):
        new_data_batch[b, :, 4] = data_batch[b, :, 0]/max_xyzi[0]
        new_data_batch[b, :, 5] = data_batch[b, :, 1]/max_xyzi[1]
        new_data_batch[b, :, 6] = data_batch[b, :, 2]/max_xyzi[2]
        new_data_batch[b, :, 7] = data_batch[b, :, 3] / max_xyzi[3]
        minx = min(data_batch[b, :, 0])  # block
        miny = min(data_batch[b, :, 1])
        minz = min(data_batch[b, :, 2])
        max_local_i = max(data_batch[b, :, 3])
        if max_local_i <= 0:
            max_local_i = 1.0
