...
channel_len = 8
NUM_CLASSES = 4
MODEL_PATH = BASE_DIR + '/../log_local_v2/model.ckpt-20'

GPU_INDEX = FLAGS.gpu
DUMP_DIR = BASE_DIR + '/../log_local_v2/dump'
...
def evaluate():
    ...
    #  load data
    data_label_folder_path = BASE_DIR + '/../../data/block_local_v2/validation'
    # data_label_folder_path = '/media/jp/disk/temp/kitti/block_local_v2/validation'
...
def eval_one_epoch(sess, ops, original_file_path, data_label_path, path_pred_out, path_gt_out):
    ...
    for batch_idx in range(num_batches):
        ...
        for i in range(NUM_POINT):
            fout.write('%f %f %f %f %d \n' % (pts[i,4], pts[i,5], pts[i,6],pts[i,7], pred[i]))
            fout_gt.write('%f %f %f %f %d \n' % (pts[i,4], pts[i,5], pts[i,6], pts[i,7], current_label[start_idx, i]))
