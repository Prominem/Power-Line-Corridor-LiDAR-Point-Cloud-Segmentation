...
channel_len = 7
NUM_CLASSES = 4
MODEL_PATH = BASE_DIR + '/../log_base/model.ckpt-20'

GPU_INDEX = FLAGS.gpu
DUMP_DIR = BASE_DIR + '/../log_base/dump'
...
def evaluate():
    ...
    #  load data
    data_label_folder_path = BASE_DIR + '/../../data/block_base/validation'
    # data_label_folder_path = '/media/jp/disk/temp/kitti/block_base/validation'
...
def eval_one_epoch(sess, ops, original_file_path, data_label_path, path_pred_out, path_gt_out):
    ...
    max_xyz = np.max(original_file[:, 0:3], axis=0)
    ...
    for batch_idx in range(num_batches):
        ...
        pts[:,4] = pts[:,4]*max_xyz[0]
        pts[:,5] = pts[:,5]*max_xyz[1]
        pts[:,6] = pts[:,6]*max_xyz[2]
        pred = pred_label[0, :]
        for i in range(NUM_POINT):
            fout.write('%f %f %f %f %d \n' % (pts[i,4], pts[i,5], pts[i,6],pts[i,3], pred[i]))
            fout_gt.write('%f %f %f %f %d \n' % (pts[i,4], pts[i,5], pts[i,6], pts[i,3], current_label[start_idx, i]))
