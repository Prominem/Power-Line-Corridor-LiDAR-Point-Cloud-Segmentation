...
def room2blocks_plus_normalized(data_label, num_point, block_size=BLOCK_SIZE, stride=STRIDE,
    ...
    """
    data = data_label[:,0:4]
    max_xyzi = np.amax(data, axis=0)
    print(max_xyzi)
    # data[:,3] /= float(max_xyzi[-1])
    label = data_label[:,-1].astype(np.uint8)
    max_room_x = max_xyzi[0]
    max_room_y = max_xyzi[1]
    max_room_z = max_xyzi[2]
    ...
    for b in range(data_batch.shape[0]):
        ...
        new_data_batch[b, :, 7] = data_batch[b, :, 3] / float(max_xyzi[-1])
        minx = min(data_batch[b, :, 0])
        miny = min(data_batch[b, :, 1])
        minz = min(data_batch[b, :, 2])
        max_i_local = np.max(data_batch[b, :, 3])
        data_batch[b, :, 0] -= (minx+block_size/2)
        data_batch[b, :, 1] -= (miny+block_size/2)
        data_batch[b, :, 2] -= minz
        data_batch[b, :, 3] /= float(max_i_local)
