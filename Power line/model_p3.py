...
def placeholder_inputs(batch_size, num_point):
    pointclouds_pl = tf.placeholder(tf.float32,
                                     shape=(batch_size, num_point, len_channel))
...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    # CONV
    net = tf_util.conv2d(input_image, 64, [1,len_channel], padding='VALID', stride=[1,1],
...
if __name__ == "__main__":
    with tf.Graph().as_default():
        a = tf.placeholder(tf.float32, shape=(32,4096,len_channel))
        ...
        with tf.Session() as sess:
            ...
            for i in range(100):
                print(i)
                sess.run(net, feed_dict={a:np.random.rand(32,4096,len_channel)})
