...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    # CONV
    net = tf_util.conv2d(input_image, 64, [1,7], padding='VALID', stride=[1,1],
...
def get_loss(pred, label):
    ...
    w_class0 = tf.constant(20.0, dtype=tf.float32)
    w_class1 = tf.constant(10.0, dtype=tf.float32)
    w_class2 = tf.constant(1.0, dtype=tf.float32)

    mask_c0 = tf.equal(label, 0)
    mask_c1 = tf.equal(label, 1)
    mask_c2 = tf.equal(label, 2)

    mask_c0 = tf.cast(mask_c0, tf.float32)
    mask_c1 = tf.cast(mask_c1, tf.float32)
    mask_c2 = tf.cast(mask_c2, tf.float32)

    loss_t = loss
    loss = tf.multiply(loss_t, mask_c0)*w_class0 + tf.multiply(loss_t, mask_c1)*w_class1 + tf.multiply(loss_t, mask_c2)*w_class2
